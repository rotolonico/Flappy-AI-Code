...
namespace IO
{
    public class PixelsRetriever : MonoBehaviour
    {
        ...
        {
            var pixels = new float[Settings.Instance.pipeDistance * (Settings.Instance.height + 2)];
            for (var x = 0; x < Settings.Instance.pipeDistance; x++)
            {
                for (var y = 0; y < Settings.Instance.height + 2; y++)
                {
                    var position = pixelsContainer.position + new Vector3(x - Settings.Instance.pipeDistance / 2, y - Settings.Instance.height / 2 - 1, 0);
                    ...
                    for (var j = 0; j < colliders.Length; j++)
                    {
                        ...
                        {
                            pixels[y * Settings.Instance.pipeDistance + x] = 0;
                            break;
                        }

                        if (col.CompareTag("Pipe") || col.CompareTag("StaticPipe"))
                        {
                            pixels[y * Settings.Instance.pipeDistance + x] = 0.5f;
                            break;
                        }

                        if (col.CompareTag("Flappy") && col.gameObject == flappy)
                        {
                            pixels[y * Settings.Instance.pipeDistance + x] = 1;
                            break;
                        }
